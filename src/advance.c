    ...
    while (globals->t < globals->maxtime)
    {
        ...
        if (globals->res_flag)
        {
            ...
            for (unsigned int i = 0; i < my_N; i++)	//!!!! Can we loop over just the links with reservoirs? Improve id_to_loc. !!!!
            {
                ...
                if (current->has_res)
                {
                    current->differential(current->last_t, current->my->list.tail->y_approx, current->dim, NULL, 0, 0, globals->global_params, current->params, current->my->forcing_values, current->qvs, current->state, current->user, current->my->list.tail->y_approx, current->h);
