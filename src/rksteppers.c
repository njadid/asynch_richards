...
double InitialStepSize(double t, Link* link_i, const GlobalVars * const globals, Workspace* workspace)
{
    ...
    link_i->differential(
        ...
        globals->global_params, link_i->params, link_i->my->forcing_values, link_i->qvs, link_i->state, link_i->user, fy0, link_i->h);
    ...
    link_i->differential(
        ...
        y_1, link_i->dim,
        parents_approx, link_i->num_parents, globals->max_dim,
        globals->global_params, link_i->params, link_i->my->forcing_values, link_i->qvs, link_i->state, link_i->user, fy1, link_i->h);
